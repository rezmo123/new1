...
class TimeSeriesPreprocessor:
        ...
        try:
            ...
            for feature in self.feature_names:
                if feature in df.columns:
                    ...
                    # Replace NaN with mean value instead of 0
                    if nan_count > 0:
                        mean_value = numeric_df[feature].mean()
                        numeric_df[feature] = numeric_df[feature].fillna(mean_value)

                    # Convert to float64 explicitly
                    numeric_df[feature] = numeric_df[feature].astype(np.float64)

                    # Validate range for percentage features
    ...
    def prepare_temporal_data(self, timeseries_df, entities_df):
        ...
        try:
            ...
            # Convert to numeric with mean imputation
            ...
            feature_data = numeric_df[self.feature_names].values.astype(np.float64)

            # Scale features while preserving outliers
            ...
            for entity_id in numeric_df['entity_id'].unique():
                ...
                for i in range(len(entity_data) - self.sequence_length + 1):
                    ...
                    if len(entity_info) > 0:
                        ...
                        category_idx = self.label_encoder.fit_transform([category])[0]
